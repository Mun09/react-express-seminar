...
const AccountPage = () => {
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { api_key: SAPIKEY, input_password: InputPassword });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { api_key: SAPIKEY, input_password: InputPassword, amount: amount });
      setNTransaction(0);
      if (!data.success) {
  ...
  const ChangePassword = () => {
    const asyncFun = async() => {
      await axios.post(SAPIBase + '/account/changeps', { api_key: SAPIKEY, input_password: InputPassword, password: Password });
      setPassword("");
    }
    asyncFun().catch((e) => window.alert(`AN ERROR OCCURED: ${e}`));
  }
  ...
  return (
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        Enter Your Name: <input type={"text"} value={SAPIKEY} onChange={e => setSAPIKEY(e.target.value)}/><br />
        Enter Your Password: <input type={"text"} value={InputPassword} onChange={e => setInputPassword(e.target.value)}/>
